...
function main()
    ...
    calc_frame = frame_num #Calcular ulitmo frame

    dump = open(dumpdir,"r") #Abrir dump

    #Leer primer frame (primer frame de referencia)

    timestep,n,L,_ = read_dump_info(dump) #Leer informacion del dump en el frame actual
    ...
    for frame in 2:frame_num
        ...
        if frame == calc_frame
            ...
            c_centers_coords = shear(c_centers_coords,-c_xy,L) #Revertir shear
    ...
    write_json(savedir,S,"structure_factor_$(direccion)")
    write_json(savedir,kvalues,"structure_factor_kvalues_$(direccion)")
