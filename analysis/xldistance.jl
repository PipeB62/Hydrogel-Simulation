...
function main()
    ...
    for frame in 1:frame_num
        ...
        if frame in calc_frames
            ...
            push!(mean_distance,mean(av_distances))
        else
            for _ in 1:n
                readline(dump)
            end
...
function test()
    dumpdir = "/media/felipe/Files/Hydrogel_sim_experiments/SizeExperiments/16k_particles/exp1/dump_shearing_16k_1.lammpstrj"
    systemdir = "/media/felipe/Files/Hydrogel_sim_experiments/SizeExperiments/16k_particles/exp1/system_formation_16k_1.data"
    bondnum,bonds = read_system_bonds(systemdir) #Leer bonds del archivo system.data
    ...
    r_c = 0.55 #Radio cutoff para determinar vecinos patch-patch
    xl_neighs = Vector{Vector{Int64}}()
    for xl in xl_list
        print(xl," ")
        push!(xl_neighs,get_neigbors(xl,patches_id,patches_coords,patches_id_periodic,patches_tree,bonds,r_c))
    end
